...
import edu.zjsr.yygh.mapper.*;
import edu.zjsr.yygh.entity.*;
import edu.zjsr.yygh.service.QuestionService;
import edu.zjsr.yygh.service.SurveyService;
import edu.zjsr.yygh.vo.QuestionVO;
...
public class SurveyServiceImpl implements SurveyService {
    ...
    @Override
    public Page<Survey> findSurvey(int page, int size) {
        Page<Survey> pageObj = new Page<>(page, size);
        QueryWrapper<Survey> queryWrapper = new QueryWrapper<>();
        queryWrapper.ne("status", 0);
        return surveyMapper.selectPage(pageObj, queryWrapper);
    }
}

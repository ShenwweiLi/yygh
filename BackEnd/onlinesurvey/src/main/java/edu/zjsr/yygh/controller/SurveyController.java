...
import edu.zjsr.yygh.annotation.PassToken;
import edu.zjsr.yygh.annotation.UserLoginToken;
import edu.zjsr.yygh.entity.Survey;
import edu.zjsr.yygh.service.RecycleService;
import edu.zjsr.yygh.service.SurveyService;
import edu.zjsr.yygh.vo.RecycleAnswerVO;
import edu.zjsr.yygh.vo.SurveyListVO;
import edu.zjsr.yygh.vo.SurveyVO;
...
//    }
    ...
    @CrossOrigin
    @Transactional(rollbackFor = Exception.class)
    public Message getSurveyList(@RequestParam Integer page,
                                       @RequestParam Integer size) {
        Message message = new Message();
        try {
...
//       List<Survey> surveyList = surveyService.findAllSurvey(page, size);
//            Integer count = surveyService.getSurveyCount();
            Page<Survey> surveyList = surveyService.findSurvey(page,size);
            JSONObject data = new JSONObject();
            data.put("surveyList", surveyList);
            System.out.println(surveyList.getTotal());
            data.put("count", surveyList.getTotal());
            message.setCode("200");
            message.setMsg("ok");
            message.setData(data);
        } catch (Exception e) {
